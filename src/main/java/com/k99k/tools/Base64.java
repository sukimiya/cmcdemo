...
public class Base64 {
	...
	private static final char fixChar = '=';
	private static final char fChar1 = '+';
	private static final char fChar2 = '/';
	...
	public static char[] encode(byte[] in, int iOff, int iLen) {
		...
		while (ip < iEnd) {
			...
			out[op++] = map1[o0];
			out[op++] = map1[o1];
			out[op] = op < oDataLen ? map1[o2] : fixChar;
			op++;
			out[op] = op < oDataLen ? map1[o3] : fixChar;
			op++;
		}
		return out;
	}
	...
	public static byte[] decode(char[] in, int iOff, int iLen) {
		...
		while (iLen > 0 && in[iOff + iLen - 1] == fixChar){
			iLen--;
		}
		int oLen = (iLen * 3) / 4;
